...
class RecorderManager:
    ...
    def __init__(self, config_manager: ConfigManager) -> None:
        ...
        self.screen_recorder = ScreenRecorder(self.config_manager)
        self.audio_recorders: List[AudioRecorder] = []
        self.show_ffmpeg_log = config_manager.get_log_config().get("ffmpeg", False)
    ...
    def start_recording(self) -> None:
        ...
        with ThreadPoolExecutor(max_workers=len(audio_devices) + 1) as executor:
            futures = []

            # Screen recording
            video_folder = os.path.join(base_folder, "screen")
            os.makedirs(video_folder, exist_ok=True)
            futures.append(
                executor.submit(
                    self.screen_recorder.start_recording, "FullScreen", video_folder
                )
            )
            ...
            for device, recorder in zip(audio_devices, self.audio_recorders):
                ...
                futures.append(
                    executor.submit(recorder.start_recording, device, audio_folder)
                )
            # Collect successful processes
            self.processes = [f.result() for f in futures if f.result() is not None]
    ...
    def _cleanup_temp_files(self) -> None:
        base_path = self.config_manager.get_storage_config()["local_path"]
        device_path = os.path.join(base_path, self.config_manager.get_device_name())
    ...
    def _move_completed_recording(self, temp_file: str, device_path: str) -> None:
        """Move completed recording to the device folder"""
        if not os.path.exists(temp_file):
            return

        new_file = os.path.join(device_path, os.path.basename(temp_file))
