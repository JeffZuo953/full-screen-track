...
class LogFileViewer(QTextEdit):
    ...
    def load_file(self, filename, chunk_size=4096):
        self.clear()
        try:
            ...
            colored_html = self.parse_color_text(content)
    ...
    def parse_color_text(self, text):
        color_pattern = re.compile(r"\033\[(\d+)m(.*?)\033\[0m")
        result = ["<pre>"]  # Use <pre> to preserve formatting
        start = 0

        for match in color_pattern.finditer(text):
            # Escape HTML in non-colored text
            non_colored_part = html.escape(text[start : match.start()])
            result.append(non_colored_part)

            color_code = int(match.group(1))
            # Escape HTML in colored text
            colored_text = html.escape(match.group(2))
            color = self.get_color_from_code(color_code)
            result.append(f'<span style="color: {color.name()};">{colored_text}</span>')
            start = match.end()
    ...
    def get_color_from_code(self, color_code):
        color_map = {
...
class Log(QWidget):
    ...
    def initUI(self):
        ...
        controls_layout.addWidget(self.file_combo, stretch=1)  # Takes 50% of space
        controls_layout.addLayout(buttons_layout, stretch=1)  # Takes 50% of space

        self.viewer = LogFileViewer()
    ...
    def clear_old_logs(self):
        ...
        dialog = CustomDialog("Confirm Delete", message, self)
